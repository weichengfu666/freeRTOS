...
#define DHT11_DELAY_US(us)  Delay_SW_Us(us)
#define DHT11_DELAY_MS(ms)  Delay_SW_Ms(ms)
...
uint8_t DHT11_Get_Humiture(float* humi,float* temp)
{
    DHT11_Data_TypeDef DHT11_Data;
    if(DHT11_Read_TempAndHumidity(&DHT11_Data) == SUCCESS)
    {
        *humi = DHT11_Data.humi_int + DHT11_Data.humi_deci*1.0/10;
        *temp = DHT11_Data.temp_int + DHT11_Data.temp_deci*1.0/10;
        return SUCCESS;
    }
    else{
        return ERROR;
